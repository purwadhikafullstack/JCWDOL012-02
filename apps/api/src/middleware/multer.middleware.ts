import { uploadProductImage, uploadUserProfile } from '@/utils/multer';
...
export const multerMiddleware = (req: Request, res: Response, next: NextFunction) => {
  uploadUserProfile(req, res, function (err) {
    if (err) {
      ...
      if (err.message === 'File type not allowed') return res.status(400).json({ message: err.message });
...
export const productImage = (req: Request, res: Response, next: NextFunction) => {
